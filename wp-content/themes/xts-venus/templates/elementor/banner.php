...
if ( ! function_exists( 'xts_banner_template' ) ) {
	...
	function xts_banner_template( $element_args ) {
		...
		if ( 'image' === $element_args['source_type'] ) {
			$source_output = xts_get_image_html( $element_args, 'image' );
		} elseif ( 'video' === $element_args['source_type'] ) {
			$video_attrs = '';
			if ( $element_args['video_poster'] ) {
				$poster_src   = xts_get_image_url( $element_args['video_poster']['id'], 'video_poster', $element_args );
				$video_attrs .= ' poster="' . esc_url( $poster_src ) . '"';
			}
			$source_output = '<video src="' . $element_args['video']['url'] . ' " autoplay muted loop playsinline ' . $video_attrs . '></video>';
		}
		...
		<div class="xts-iimage<?php echo esc_attr( $wrapper_classes ); ?>" onclick="<?php echo esc_js( $onclick ); ?>">
			...
			<div class="xts-iimage-img-wrapper">
				<div class="xts-iimage-img<?php echo esc_attr( $image_classes ); ?>">
					<?php echo wp_kses( apply_filters( 'xts_iimage', $source_output ), true ); ?>
				</div>
