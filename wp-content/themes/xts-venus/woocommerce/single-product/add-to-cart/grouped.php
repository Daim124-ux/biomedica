...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<table class="woocommerce-grouped-product-list group_table shop_table_responsive">
		<tbody>
		<?php
			...
			foreach ( $grouped_product_columns as $column_id ) {
				...
				switch ( $column_id ) {
					case 'quantity':
						...
						$value            = ob_get_clean();
						$xts_data_title   = 'data-title="' . esc_attr__( 'Quantity', 'xts-theme' ) . '"';
						$xts_custom_class = ' product-quantity';
						break;
					case 'label':
						...
						$value           .= '</label>';
						$xts_custom_class = ' product-name';
						break;
					case 'price':
						$value            = $grouped_product_child->get_price_html() . wc_get_stock_html( $grouped_product_child );
						$xts_data_title   = 'data-title="' . esc_attr__( 'Price', 'xts-theme' ) . '"';
						$xts_custom_class = ' price';
						break;
					case 'thumbnail':
						$value            = '';
						$xts_data_title   = '';
						$xts_custom_class = ' product-thumbnail';
						break;
					default:
						$value = '';
						break;
				...
				echo '<td class="woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . esc_attr( $xts_custom_class ) . '" ' . $xts_data_title . '>' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</td>'; // phpcs:ignore WPCS: XSS ok.
