...
if ( ! function_exists( 'xts_get_html_block_content' ) ) {
	...
	function xts_get_html_block_content( $id ) {
		...
		if ( 'enabled' === $negative_gap ) {
			...
			add_action( 'elementor/frontend/container/before_render', 'xts_add_container_class_if_content_width', 1000 );
			...
			remove_action( 'elementor/frontend/container/before_render', 'xts_add_container_class_if_content_width', 1000 );
		}

		if ( xts_is_elementor_installed() && Plugin::$instance->documents->get( $id )->is_built_with_elementor() ) {
