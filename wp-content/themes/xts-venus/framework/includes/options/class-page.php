...
class Page extends Singleton {
	...
	public function admin_bar_links( $admin_bar ) {
		...
		$product          = xts_is_woocommerce_installed() ? wc_get_product() : false;
		$active_discounts = xts_get_opt( 'dynamic_discounts_enabled' ) && ! empty( $product ) ? Dynamic_Discounts_Manager::get_instance()->get_discount_rules( $product ) : array();
		if ( ! empty( $active_discounts ) && is_product() ) {
			$admin_bar->add_node(
				array(
					'id'     => 'xts_woo_discounts_active',
					'title'  => '<i class="xf-general"></i>' . esc_html__( 'Active discount', 'xts-theme' ),
					'href'   => admin_url( 'edit.php?post_type=xts_woo_discounts' ),
					'parent' => 'xts_dashboard',
					'meta'   => array(
						'title' => esc_html__( 'Active discounts', 'xts-theme' ),
					),
				)
			);
			$admin_bar->add_node(
				array(
					'id'     => 'xts_woo_discounts_active_' . $active_discounts['post_id'],
					'title'  => $active_discounts['title'],
					'href'   => get_edit_post_link( $active_discounts['post_id'] ),
					'parent' => 'xts_woo_discounts_active',
					'meta'   => array(
						'title' => $active_discounts['title'],
					),
				)
		...
		$admin_bar->add_node(
			array(
				'id'     => 'xts_patcher',
				'title'  => '<span class="xf-patcher"></span>' . esc_html__( 'Patcher', 'xts-theme' ),
							...
							<div class="xts-sections">
								...
								<div class="xts-option-header">
									<div class="xts-options-search">
										<input type="text" placeholder="<?php esc_attr_e( 'Start typing to find options...', 'xts-theme' ); ?>">
									</div>
								</div>
					...
					<?php if ( ! xts_get_opt( 'white_label', '0' ) ) : ?>
						<div class="xts-dashboard-video">
							<div class="xts-info">
								...
								<a href="<?php echo esc_url( XTS_DOCS_URL ); ?>theme-settings-presets-2" class="xts-inline-btn" target="_blank">
	...
	private function is_inherit_field( $id ) {
		$fields_to_save = ! empty( $this->get_fields_to_save() ) ? explode( ',', $this->get_fields_to_save() ) : array();
