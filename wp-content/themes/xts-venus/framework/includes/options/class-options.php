...
class Options extends Singleton {
	 ...
	 * @var array
	 */
	private static $_options; // phpcs:ignore
	 ...
	 * @var array
	 */
	private static $_controls_classes = array( // phpcs:ignore
	...
	/**
	 * Add option key to be overridden from meta value.
	 *
	 * @since 1.0.0
	 *
	...
	public function fields_css_output() {
		if ( 'inline' !== apply_filters( 'xts_css_output', 'file' ) ) {
			return;
		}

		$output = $this->get_css_output();
	...
	public function sanitize_before_save( $options ) {
		...
		if ( isset( $options['preset'] ) && $options['preset'] ) {
			...
			$fields_to_save = ! empty( $options['fields_to_save'] ) ? explode( ',', $options['fields_to_save'] ) : array();
