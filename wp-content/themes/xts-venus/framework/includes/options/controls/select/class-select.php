...
class Select extends Field {
	...
	public function render_control() {
		...
		if ( ! $options && ! $this->has_empty_option() && ! $this->is_autocomplete() ) {
			esc_html_e( 'Options for this field are not provided in the map function.', 'xts-theme' );
			return;
		}

		?>
		<select class="xts-select<?php echo esc_attr( $classes ); ?>" name="<?php echo esc_attr( $name ); ?>" <?php echo wp_kses( $attributes, true ); ?>>
			<?php if ( $this->has_empty_option() ) : ?>
				<option value="0"><?php esc_html_e( 'Select', 'xts-theme' ); ?></option>
			<?php endif; ?>
		...
		<?php if ( $this->is_multiple() && ! $this->is_autocomplete() ) : ?>
			<div class="xts-select2-all-wrap">
				<a href="#" class="xts-btn xts-select2-all">
					<?php esc_html_e( 'Select all', 'xts-theme' ); ?>
				</a>

				<a href="#" class="xts-btn-bordered xts-btn-primary xts-deselect2-all">
					<?php esc_html_e( 'Deselect all', 'xts-theme' ); ?>
				</a>
			</div>
		<?php endif; ?>
		<?php
