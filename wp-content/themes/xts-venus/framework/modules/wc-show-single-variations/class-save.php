...
namespace XTS\Modules\Wc_Show_Single_Variations;
...
class Save extends Singleton {
	...
	public function save_product( $post_id, $post, $update ) {
		...
		$taxonomies = apply_filters( 'xts_single_variations_taxonomies', array( 'product_cat', 'product_tag' ), $post_id );
		$post_metas = apply_filters( 'xts_single_variations_post_meta', array( '_xts_product_label_new' ), $post_id );
		...
		foreach ( $variation_ids as $variation_id ) {
			...
			if ( isset( $this->request_data()['_xts_exclude_show_single_variation'] ) && ( $this->request_data()['_xts_exclude_show_single_variation'] || get_post_meta( $post_id, '_xts_exclude_show_single_variation', true ) ) ) {
				update_post_meta( $variation_id, '_xts_show_variation', '1' === $this->request_data()['_xts_exclude_show_single_variation'] ? 'no' : '' );
			}
	...
	public function save_variation( $variation_id, $i = false ) {
		...
		if ( isset( $request_data['xts_additional_variation_images'][ $variation_id ] ) ) {
			update_post_meta( $variation_id, '_product_image_gallery', $request_data['xts_additional_variation_images'][ $variation_id ] );
		} elseif ( isset( $request_data['xts_variation_gallery'][ $variation_id ] ) ) {
			update_post_meta( $variation_id, '_product_image_gallery', $request_data['xts_variation_gallery'][ $variation_id ] );
		}
		...
		if ( isset( $request_data['xts_show_variation'][ $i ] ) ) {
			$show_variation = $request_data['xts_show_variation'][ $i ];
		}

		update_post_meta( $variation_id, '_xts_show_variation', $show_variation );
	...
	public function update_variation_option( $variation_id ) {
		$updated_gallery_ids = get_post_meta( $variation_id, 'xts_additional_variation_images_data', true );
