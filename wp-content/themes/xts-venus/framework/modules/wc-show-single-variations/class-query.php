...
class Query extends Singleton {
	...
	public function init() {
		...
		add_action( 'xts_quick_view_posts_args', array( $this, 'add_variations_to_quickview' ), 10, 1 );
	...
	public function posts_clauses( $clauses, $query ) {
		...
		if ( ! empty( $query->query_vars['xts_single_variations_filter'] ) ) {
			if ( xts_get_opt( 'hide_variation_parent' ) && ( ! xts_get_opt( 'wishlist_page' ) || get_queried_object_id() !== (int) xts_get_opt( 'wishlist_page' ) && empty( $_POST['atts']['is_wishlist'] ) ) ) { //phpcs:ignore
			...
			if ( strripos( $clauses['where'], 'wp_wc_product_attributes_lookup' ) ) {
				if ( xts_get_opt( 'hide_variation_parent' ) ) {
			...
			$clauses['where'] .= " AND {$wpdb->posts}.ID NOT IN (
			    ...
			    WHERE $wpdb->postmeta.meta_key = '_xts_show_variation' AND $wpdb->postmeta.meta_value = 'no'
			)";
	...
	public function add_variations_to_query( $query ) {
		...
		$query->set( 'xts_single_variations_filter', 'yes' );
	}
	...
	public function add_variations_to_product_query( $query ) {
		if ( ( is_admin() && ( ! isset( $_REQUEST['action'] ) ) ) || isset( $query->query['product'] ) || ! empty( $query->query['xts_show_variable_products'] ) ) { //phpcs:ignore
			return;
		}
