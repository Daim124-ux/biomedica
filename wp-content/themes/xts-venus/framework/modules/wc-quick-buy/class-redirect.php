...
class Redirect extends Singleton {
	...
	public function add_to_cart_action() {
		if ( ! isset( $_REQUEST['xts-add-to-cart'] ) || ! is_numeric( wp_unslash( $_REQUEST['xts-add-to-cart'] ) ) ) { //phpcs:ignore
			return;
		}
		...
		if ( ! isset( $_REQUEST['add-to-cart'] ) || $_REQUEST['add-to-cart'] !== $_REQUEST['xts-add-to-cart'] ) { //phpcs:ignore
			$_REQUEST['add-to-cart'] = $_REQUEST['xts-add-to-cart']; //phpcs:ignore
		}
	...
	public function quick_buy_redirect( $url ) {
		if ( ! isset( $_REQUEST['xts-add-to-cart'] ) ) { //phpcs:ignore
			return $url;
		}

		$product_id = sanitize_text_field( $_REQUEST['xts-add-to-cart'] ); //phpcs:ignore
	...
	public function get_redirect_url( $product_id ) {
		$url      = array( 'url' => '' );
		$redirect = xts_get_opt( 'buy_now_redirect', 'checkout' );
