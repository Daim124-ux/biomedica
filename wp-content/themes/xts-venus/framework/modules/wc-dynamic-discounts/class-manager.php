...
class Manager extends Singleton {
	 ...
	 *
	 * @var string $xts_transient_discounts_ids .
	 */
	public $xts_transient_discounts_ids = 'xts_all_discounts_post_ids';
	...
	public function get_all_discounts_post_ids() {
		$cache = get_transient( $this->xts_transient_discounts_ids );
		...
		$all_discounts_post_ids = get_posts(
			array(
				'fields'         => 'ids',
				'posts_per_page' => -1,
				'post_type'      => 'xts_woo_discounts',
			)
		);

		set_transient( $this->xts_transient_discounts_ids, $all_discounts_post_ids );
	...
	public function sort_by_priority( $a, $b ) {
		return $b['xts_discount_priority'] <=> $a['xts_discount_priority'];
	}
	...
	public function get_discount_rules( $product ) {
		...
		foreach ( $all_discount_rules as $discounts_id => $discount_rules ) {
			...
			$discount_post             = get_post( $discounts_id );
			$discount_rules['post_id'] = $discount_post->ID;
			$discount_rules['title']   = $discount_post->post_title;
	...
	public function check_discount_condition( $discount_rules, $product ) {
		...
		foreach ( $conditions as $discount_id => $condition ) {
			$conditions[ $discount_id ]['xts_discount_priority'] = $this->get_condition_priority( $condition['type'] );
		}
	...
	public function get_condition_priority( $type ) {
		...
		return apply_filters( 'xts_dynamic_price_condition_priority', $priority, $type );
	}
	...
	public function get_template( $template_name, $args = array() ) {
		...
		include XTS_FRAMEWORK_ABSPATH . 'modules/wc-dynamic-discounts/templates/' . $template_name . '.php';
	}
}
