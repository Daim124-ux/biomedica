...
class Render extends Singleton {
	...
	public function init() {
		add_action( 'wp_ajax_xts_purchasable_fbt_products', array( $this, 'purchasable_fbt_products' ) );
		add_action( 'wp_ajax_nopriv_xts_purchasable_fbt_products', array( $this, 'purchasable_fbt_products' ) );
		...
		add_filter( 'xts_show_widget_cart_item_quantity', array( $this, 'widget_cart_item_quantity' ), 10, 2 );
	...
	public function purchasable_fbt_products() {
		...
		$main_product_discount     = get_post_meta( $bundle_id, '_xts_main_products_discount', true );
		$item_keys                 = array();
		...
		$main_key_item = WC()->cart->add_to_cart(
			...
			array(
				'xts_fbt_parent_id'       => $main_product->get_id(),
				'xts_fbt_discount'        => $main_product_discount,
				'xts_fbt_bundle_id'       => $bundle_id,
				'xts_fbt_keys'            => array(),
				'xts_fbt_product_ids'     => $products_id,
				'xts_fbt_bundle_modified' => get_the_modified_date( 'U', $bundle_id ),
			)
		);
		...
		foreach ( $fbt_products as $fbt_product ) {
			...
			$item_keys[] = WC()->cart->add_to_cart(
				...
				array(
					'xts_fbt_parent_id'   => $main_product->get_id(),
					'xts_fbt_discount'    => $fbt_product['discount'],
					'xts_fbt_bundle_id'   => $bundle_id,
					'xts_fbt_parent_keys' => $main_key_item,
				)
			);
		}
		...
		WC()->cart->cart_contents[ $main_key_item ]['xts_fbt_keys'] = $item_keys;
		WC()->cart->set_session();

		xts_single_product_ajax_add_to_cart();
	}
	...
	public function before_calculate_totals( $cart_object ) {
		...
		foreach ( $cart_object->cart_contents as $cart_item ) {
			if ( ! empty( $cart_item['xts_fbt_parent_keys'] ) && empty( $cart_object->cart_contents[ $cart_item['xts_fbt_parent_keys'] ] ) ) {
			...
			if ( isset( $cart_item['xts_fbt_bundle_id'], $cart_item['xts_fbt_bundle_modified'] ) && ( get_the_modified_date( 'U', $cart_item['xts_fbt_bundle_id'] ) !== $cart_item['xts_fbt_bundle_modified'] || 'publish' !== get_post_status( $cart_item['xts_fbt_bundle_id'] ) ) ) {
				$this->update_data_bundle_product( $cart_item );
			}
		}

		foreach ( $cart_object->cart_contents as $cart_item ) {
			if ( isset( $cart_item['data'], $cart_item['xts_fbt_discount'] ) && $cart_item['xts_fbt_discount'] && $cart_item['data'] instanceof WC_Product ) {
				...
				$price = apply_filters( 'xts_fbt_set_product_cart_price', $price, $cart_item );

				$item_price = $price - ( ( $price / 100 ) * (float) $cart_item['xts_fbt_discount'] );
	...
	private function update_data_bundle_product( $cart_item ) {
		...
		$fbt_products       = get_post_meta( $cart_item['xts_fbt_bundle_id'], '_xts_fbt_products', true );
		$main_discount      = get_post_meta( $cart_item['xts_fbt_bundle_id'], '_xts_main_products_discount', true );
		$show_checkbox      = get_post_meta( $cart_item['xts_fbt_bundle_id'], '_xts_allow_customize', true );
		...
		$bundles_id = get_post_meta( $cart_item['xts_fbt_parent_id'], '_xts_fbt_bundles_id', true );

		if ( in_array( $cart_item['xts_fbt_parent_id'], array_column( $fbt_products, 'id' ) ) ) { //phpcs:ignore
			--$fbt_products_count;
		}

		if ( 'publish' !== get_post_status( $cart_item['xts_fbt_bundle_id'] ) || ! $show_checkbox && $fbt_products_count !== count( $cart_item['xts_fbt_keys'] ) || ! in_array( $cart_item['xts_fbt_bundle_id'], $bundles_id ) ) { //phpcs:ignore
			foreach ( $cart_item['xts_fbt_keys'] as $fbt_keys ) {
		...
		if ( $main_discount && $cart_item['xts_fbt_discount'] !== $main_discount ) {
			WC()->cart->cart_contents[ $cart_key ]['xts_fbt_discount'] = $main_discount;
		}

		$fbt_products = array_combine( array_column( $fbt_products, 'id' ), $fbt_products );

		foreach ( $cart_item['xts_fbt_keys'] as $key ) {
			...
			if ( (int) $cart_object->cart_contents[ $key ]['xts_fbt_discount'] !== $discount ) {
				WC()->cart->cart_contents[ $key ]['xts_fbt_discount'] = $discount;
			}
		}

		WC()->cart->cart_contents[ $cart_key ]['xts_fbt_bundle_modified'] = get_the_modified_date( 'U', $cart_item['xts_fbt_bundle_id'] );
		WC()->cart->set_session();
	}
	...
	public function cart_item_price( $price, $cart_item ) {
		if ( isset( $cart_item['xts_fbt_parent_id'], $cart_item['xts_fbt_discount'] ) && $cart_item['xts_fbt_discount'] && ! is_cart() ) {
			if ( ! empty( $cart_item['variation_id'] ) ) {
				$variation_product = wc_get_product( $cart_item['variation_id'] );
			...
			$new_price = $product_price - ( ( $product_price / 100 ) * (float) $cart_item['xts_fbt_discount'] );
	...
	public function cart_item_remove_link( $link, $cart_item_key ) {
		...
		if ( isset( $item['xts_fbt_bundle_id'], $item['xts_fbt_parent_id'] ) && $item['xts_fbt_parent_id'] !== $item['product_id'] ) {
			return '';
		}
	...
	public function update_cart_item_quantity( $cart_item_key, $quantity, $old_quantity, $cart ) {
		if ( isset( $_REQUEST['action'] ) && 'xts_purchasable_fbt_products' === $_REQUEST['action'] ) { //phpcs:ignore
			return;
		}
		...
		if ( ! empty( $cart_items[ $cart_item_key ]['xts_fbt_keys'] ) ) {
			$item_key = $cart_items[ $cart_item_key ]['xts_fbt_keys'];
		} elseif ( ! empty( $cart_items[ $cart_item_key ]['xts_fbt_parent_keys'] ) && ! empty( $cart_items[ $cart_items[ $cart_item_key ]['xts_fbt_parent_keys'] ]['xts_fbt_keys'] ) ) {
			$item_key   = $cart_items[ $cart_items[ $cart_item_key ]['xts_fbt_parent_keys'] ]['xts_fbt_keys'];
			$item_key[] = $cart_items[ $cart_item_key ]['xts_fbt_parent_keys'];
		}
	 ...
	 * @param array   $cart_item Product.
	 *
	 * @return false
	 */
	public function widget_cart_item_quantity( $show, $cart_item ) {
		if ( isset( $cart_item['xts_fbt_bundle_id'], $cart_item['xts_fbt_parent_id'] ) && $cart_item['xts_fbt_parent_id'] !== $cart_item['product_id'] ) {
			return false;
		}
	...
	public function cart_item_class( $classes, $cart_item, $cart_item_key ) {
		if ( ! empty( $cart_item['xts_fbt_parent_keys'] ) && isset( WC()->cart->cart_contents[ $cart_item['xts_fbt_parent_keys'] ] ) ) {
			$parent_product = WC()->cart->cart_contents[ $cart_item['xts_fbt_parent_keys'] ];

			if ( ! empty( $parent_product['xts_fbt_keys'] ) && count( $parent_product['xts_fbt_keys'] ) === array_search( $cart_item_key, $parent_product['xts_fbt_keys'], true ) + 1 ) {
				$classes .= ' xts-fbt-item-last';
			} else {
				$classes .= ' xts-fbt-item';
			}
		} elseif ( ! empty( $cart_item['xts_fbt_keys'] ) ) {
			$classes .= ' xts-fbt-item-first';
		}
	...
	public function cart_item_name( $item_name, $item ) {
		if ( ! empty( $item['xts_fbt_parent_id'] ) ) {
			ob_start();
			...
			?>
			<span class="xts-fbt-label" data-xts-tooltip="<?php esc_html_e( 'Bundled product', 'xts-theme' ); ?>"></span>
			<?php
	...
	public function cart_item_removed( $cart_item_key, $cart ) {
		...
		if ( ! empty( $cart_data[ $cart_item_key ]['xts_fbt_keys'] ) ) {
			$item_key = $cart_data[ $cart_item_key ]['xts_fbt_keys'];
		} elseif ( ! empty( $cart_data[ $cart_item_key ]['xts_fbt_parent_keys'] ) ) {
			$item_key   = $cart->cart_contents[ $cart_data[ $cart_item_key ]['xts_fbt_parent_keys'] ]['xts_fbt_keys'];
			$item_key[] = $cart_data[ $cart_item_key ]['xts_fbt_parent_keys'];
		}
	...
	public function restore_cart_items( $cart_item_key, $cart ) {
		...
		if ( ! empty( $cart_data[ $cart_item_key ]['xts_fbt_keys'] ) ) {
			...
			$item_key   = $cart->removed_cart_contents[ $cart_data[ $cart_item_key ]['xts_fbt_parent_keys'] ]['xts_fbt_keys'];
			$item_key[] = $cart_data[ $cart_item_key ]['xts_fbt_parent_keys'];
		}
	...
	public function get_cart_item_from_session( $cart_item, $item_session_values ) {
		if ( isset( $item_session_values['xts_fbt_parent_id'] ) ) {
			$cart_item['xts_fbt_parent_id'] = $item_session_values['xts_fbt_parent_id'];
			$cart_item['xts_fbt_discount']  = $item_session_values['xts_fbt_discount'];
			$cart_item['xts_fbt_bundle_id'] = $item_session_values['xts_fbt_bundle_id'];
		}

		if ( isset( $item_session_values['xts_fbt_keys'] ) ) {
			$cart_item['xts_fbt_keys'] = $item_session_values['xts_fbt_keys'];
		}

		if ( isset( $item_session_values['xts_fbt_parent_keys'] ) ) {
			$cart_item['xts_fbt_parent_keys'] = $item_session_values['xts_fbt_parent_keys'];
		}

		if ( isset( $item_session_values['xts_fbt_bundle_modified'] ) ) {
			$cart_item['xts_fbt_bundle_modified'] = $item_session_values['xts_fbt_bundle_modified'];
		}
