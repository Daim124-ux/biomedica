...
class OpauthStrategy {
	...
	private function shipToCallback( $data, $transport = null ) {
		...
		switch ( $transport ) {
			case 'get':
				$this->redirect( $this->env['callback_url'] . '?' . http_build_query( array( 'opauth' => xts_compress( json_encode( $data ) ) ), '', '&' ) );
				break;
			case 'post':
				$this->clientPost( $this->env['callback_url'], array( 'opauth' => xts_compress( json_encode( $data ) ) ) );
				break;
			case 'session':
	...
	public static function clientPost( $url, $data = array() ) {
		?>
		<html>
			<body onload="<?php echo postit(); ?>">
				<form name="auth" method="post" action="<?php echo esc_attr( $url ); ?>">
				<?php if ( ! empty( $data ) && is_array( $data ) ) : ?>
					<?php $flat = self::flattenArray( $data ); ?>
	...
	public static function httpRequest( $url, $options = null, &$responseHeaders = null ) {
		if ( xts_get_opt( 'alt_social_auth_method' ) ) {
			...
			if ( ! defined( 'XTS_CORE_VERSION' ) ) {
				return '';
			}

			$content         = xts_get_content_file( $url, false, $context );
