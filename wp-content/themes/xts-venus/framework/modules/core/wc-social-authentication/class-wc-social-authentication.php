...
class WC_Social_Authentication extends Module {
		...
		<?php if ( ( $google_app_id && $google_app_secret ) || ( $facebook_app_id && $facebook_app_secret ) || ( $vkontakte_app_id && $vkontakte_app_secret ) ) : ?>
			...
			<ul class="xts-social-login">
				<?php if ( $facebook_app_id && $facebook_app_secret ) : ?>
					<li>
						<a rel="nofollow" href="<?php echo esc_url( add_query_arg( 'social_auth', 'facebook',
							$redirect_url ) ); ?>" class="xts-login-fb xts-button">
				...
				<?php if ( $google_app_id && $google_app_secret ) : ?>
					<li>
						<a rel="nofollow" href="<?php echo esc_url( add_query_arg( 'social_auth', 'google', $redirect_url
						) ); ?>" class="xts-login-goo xts-button">
				...
				<?php if ( $vkontakte_app_id && $vkontakte_app_secret ) : ?>
					<li>
						<a rel="nofollow" href="<?php echo esc_url( add_query_arg( 'social_auth', 'vkontakte',
							$redirect_url ) ); ?>" class="xts-login-vk xts-button">
	...
	public function authentication_callback() {
		...
		$response = json_decode( xts_decompress( $_GET['opauth'] ), true );
		...
		if ( ! $opauth->validate( sha1( print_r( $response['auth'], true) ), $response['timestamp'], $response['signature'] ) ) {
			wp_redirect( $this->get_account_url() );
			exit;
		}
	...
	public function add_options() {
		...
		Options::add_field(
			...
			Then go to APP settings and copy App ID and App Secret there. You also need to insert Redirect URI like this example <strong>' . get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) . 'facebook/int_callback</strong> More information you can get in our <a href="' . esc_url( XTS_DOCS_URL ) . 'how-to-configure-facebook-login-3/" target="_blank">documentation</a>.',
		...
		Options::add_field(
			...
			Make sure to go to API Access tab and Create an OAuth 2.0 client ID. Choose Web application for Application type. Make sure that redirect URI is set to actual OAuth 2.0 callback URL, usually <strong>' . get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) . 'google/oauth2callback </strong> More information you can get in our <a href="' . esc_url( XTS_DOCS_URL ) . 'how-to-configure-google-login-3" target="_blank">documentation</a>.',
